...
otp_storage = {}  # {email: {'otp': '123456', 'timestamp': time.time(), 'user_data': {...}}}
...
def send_otp_email(email, otp):
    try:
        ...
        msg['To'] = email
        msg['Subject'] = 'Your OTP Code for Sign In'
        
        body = f"""
        <html>
            <body>
                <h2>Your OTP Code</h2>
                <p>Your 6-digit OTP code for sign in is:</p>
...
# Login route - normal login (no OTP)
    ...
    try:
        with get_db() as conn:
            ...
            c.execute('INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)',
                     (user_id, signup_data['name'], signup_data['email'], signup_data['password']))
            conn.commit()
            
            session['user_id'] = user_id
            session['user_name'] = signup_data['name']
